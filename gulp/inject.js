...
gulp.task('injectAuth', ['stylesAuth'], function () {
  return injectAlone({
    css: [path.join('!' + conf.paths.tmp, '/serve/app/vendor.css'), path.join(conf.paths.tmp, '/serve/app/home.css')],
    paths: [path.join(conf.paths.src, '/home.html'), path.join(conf.paths.src, '/reg.html')]
  })
});
